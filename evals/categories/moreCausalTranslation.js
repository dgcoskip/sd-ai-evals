    ...
    */
    if (current.charAt(current.length - 1) == ">" && current.charAt(0) == "<") {
      if (current == "<H>" || current == "<E>") {
        ...
        if (toInProgress.length > 0  && fromInProgress.length > 0 && polarity != "") {
          //console.log("attemping a push")
          relationships.push({to: toInProgress.join(" "), from: fromInProgress.join(" "), polarity})
...
const parseRelationships = function(filePath) {
    const df = dataForge.readFileSync(filePath).parseCSV().withSeries({
    ...
    return df.toArray().map((row, index) => {
        return {
            ...
            additionalParameters: {
                ...
                backgroundKnowledge: row.Sentence,
                mainTopics: "",
                depth: 1
            },
...
export const groups = {
    "suicide": parseRelationships('./evals/categories/data/suicide_subgraphs.csv').slice(0,3),
    "lake": parseRelationships('./evals/categories/data/virginia_tech_subgraphs.csv').slice(0,3),
    "sd": parseRelationships('./evals/categories/data/published_vignettes_subgraphs.csv').slice(0,3),
    "student": parseRelationships('./evals/categories/data/student_subgraphs.csv').slice(0,3),
}